...
#        echo PARAMS: ${DATABASE} ${TMP_DATABASE} ${PATH_TO_TMP} ${environment}
	...
	for filename in ${dir}/*.hql; do
		beeline -u "${hive2_jdbc_url};principal=${hive2_server_principal}" -f "${filename}" --hivevar DATABASE="${DATABASE}" --hivevar TMP_DATABASE="${TMP_DATABASE}" --hivevar PATH_TO_TMP="${PATH_TO_TMP}" --hivevar environment="${environment}" --hivevar environment_path="${environment_path}"
	done
done